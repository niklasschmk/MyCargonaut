...
myOffers: Observable<Offer[]>;
  ...
  getMyOffers(): Promise<Offer[]>{
    return new Promise<Offer[]>((resolve, reject) => {
      const offers: Offer[] = [];
      this.afs.collection<Offer>('offer', ref =>
        ref
  ...
  startRide(offerId: string, bookedBy: string): Promise<string> {
  ...
  getMyRides(): Promise<Ride[]>{
    return new Promise<Ride[]>((resolve, reject) => {
      const myRides: Ride[] = [];
      this.afs.collection<Ride>('ride', ref =>
      ref
        .where('rideId', '==',this.authService.userId)
      ).ref.get().then(docSnaps => {
        for(const docSnap of docSnaps.docs){
